...
void check_occurrence(boost::program_options::variables_map& vm, boost::program_options::options_description& d) {
    VINA_FOR_IN(i, d.options()) {
        ...
        if(!vm.count(str))
            vina_std_err << "Required parameter --" << str << " is missing!\n";
    }
}
...
void vina_options(vina_options_desc_t &desc, vina_options_desc_t &desc_config, vina_options_desc_t &desc_simple,
    ...
    options_description inputs("Input", 120);
    inputs.add_options()
                                ("receptor", value<std::string>(&args.rigid_name), "rigid part of the receptor (PDBQT)")
                                ("flex", value<std::string>(&args.flex_name), "flexible side chains, if any (PDBQT)")
                                ;
    //options_description search_area("Search area (required, except with --score_only)");
//    search_area = options_description("Search space (required)");
    search_area.add_options()
                                ("center_x", value<fl>(&args.center_x), "X coordinate of the center")
                                ("center_y", value<fl>(&args.center_y), "Y coordinate of the center")
                                ("center_z", value<fl>(&args.center_z), "Z coordinate of the center")
                                ("size_x", value<fl>(&args.size_x), "size in the X dimension (Angstroms)")
                                ("size_y", value<fl>(&args.size_y), "size in the Y dimension (Angstroms)")
                                ("size_z", value<fl>(&args.size_z), "size in the Z dimension (Angstroms)")
                                ;

    options_description advanced("Advanced options (see the manual)", 120);
    advanced.add_options()
                                ("score_only",     bool_switch(&args.score_only),     "score only - search space can be omitted")
                                ("local_only",     bool_switch(&args.local_only),     "do local search only")
                                ("randomize_only", bool_switch(&args.randomize_only), "randomize input, attempting to avoid clashes")
                                ("weight_gauss1", value<fl>(&args.weight_gauss1)->default_value(args.weight_gauss1),                "gauss_1 weight")
                                ("weight_gauss2", value<fl>(&args.weight_gauss2)->default_value(args.weight_gauss2),                "gauss_2 weight")
                                ("weight_repulsion", value<fl>(&args.weight_repulsion)->default_value(args.weight_repulsion),       "repulsion weight")
                                ("weight_hydrophobic", value<fl>(&args.weight_hydrophobic)->default_value(args.weight_hydrophobic), "hydrophobic weight")
                                ("weight_hydrogen", value<fl>(&args.weight_hydrogen)->default_value(args.weight_hydrogen),          "Hydrogen bond weight")
                                ("weight_rot", value<fl>(&args.weight_rot)->default_value(args.weight_rot),                         "N_rot weight")
                                ;
    options_description misc("Misc (optional)", 120);
    misc.add_options()
                                ("cpu", value<int>(&args.cpu), "the number of CPUs to use (the default is to try to detect the number of CPUs or, failing that, use 1)")
                                ("seed", value<int>(&args.seed), "explicit random seed")
                                ("exhaustiveness", value<int>(&args.exhaustiveness)->default_value(8), "exhaustiveness of the global search (roughly proportional to time): 1+")
                                ("num_modes", value<int>(&args.num_modes)->default_value(9), "maximum number of binding modes to generate")
                                ("energy_range", value<fl>(&args.energy_range)->default_value(3.0), "maximum energy difference between the best binding mode and the worst one displayed (kcal/mol)")
                                ;
    options_description config("Configuration file (optional)", 120);
    config.add_options()
                                ("config", value<std::string>(&args.config_name), "the above options can be put here")
                                ;
    options_description info("Information (optional)", 120);
    info.add_options()
                                ("version",       bool_switch(&args.version), "display program version")
                                ;
//    desc = options_description();
//    desc_config = options_description();
//    desc_simple = options_description();
    desc       .add(inputs).add(search_area).add(advanced).add(misc).add(config).add(info);
    desc_config.add(inputs).add(search_area).add(advanced).add(misc);
    desc_simple.add(inputs).add(search_area).add(misc).add(config).add(info);
}
...
int parse_conf_options(vina_options_desc_t &desc_config, vina_options_desc_t &desc_simple, variables_map &vm,
    ...
    if (vm.count("config")) {
        try {
            path name = make_path(args.config_name);
            ifile config_stream(name);
            store(parse_config_file(config_stream, desc_config), vm);
            notify(vm);
        } catch (boost::program_options::error &e) {
            vina_std_err << "Configuration file parse error: " << e.what() << '\n'
                    << "\nCorrect usage:\n" << desc_simple << '\n';
            return 1;
        }
    }
...
#################################################################\n";
    try {
        if(args.version) {
            vina_std_out << version_string << '\n';
            return 1;
        }

        bool search_box_needed = !args.score_only; // randomize_only and local_only still need the search space
        bool output_produced   = !args.score_only;
        bool receptor_needed   = !args.randomize_only;

        if(receptor_needed) {
            if(vm.count("receptor") <= 0) {
                vina_std_err << "Missing receptor.\n" << "\nCorrect usage:\n" << desc_simple << '\n';
                return 1;
            }
        }
        if(args.ligand_Q) {
            vina_std_err << "Missing ligand.\n" << "\nCorrect usage:\n" << desc_simple << '\n';
            return 1;
        }
        if(args.cpu < 1)
            args.cpu = 1;
        ...
        sz max_modes_sz = static_cast<sz>(args.num_modes);

        boost::optional<std::string> rigid_name_opt;
        if(vm.count("receptor"))
            rigid_name_opt = args.rigid_name;

        boost::optional<std::string> flex_name_opt;
        if(vm.count("flex"))
            flex_name_opt = args.flex_name;
        ...
        if(search_box_needed) {
            ...
            if(args.size_x <= 0 || args.size_y <= 0 || args.size_z <= 0)
                throw usage_error("Search space dimensions should be positive");
        }

        log << cite_message << '\n';

        if(search_box_needed && args.size_x * args.size_y * args.size_z > 27e3) {
            log << "WARNING: The search space volume > 27000 Angstrom^3 (See FAQ)\n";
        }

        if(output_produced) { // FIXME
//            if(!vm.count("out")) {
//                args.out_name = default_output(args.ligand_name);
//                log << "Output will be " << args.out_name << '\n';
//            }
        }
        ...
        flv weights;
        weights.push_back(args.weight_gauss1);
        weights.push_back(args.weight_gauss2);
        weights.push_back(args.weight_repulsion);
        weights.push_back(args.weight_hydrophobic);
        weights.push_back(args.weight_hydrogen);
        weights.push_back(5 * args.weight_rot / 0.1 - 1); // linearly maps onto a different range, internally. see everything.cpp

        if(search_box_needed) {
            const fl granularity = 0.375;
            vec span(args.size_x,   args.size_y,   args.size_z);
            vec center(args.center_x, args.center_y, args.center_z);
            VINA_FOR_IN(i, gd) {
        ...
        if(args.cpu < 1)
            args.cpu = 1;
        if(args.verbosity > 1 && args.exhaustiveness < args.cpu)
            log << "WARNING: at low exhaustiveness, it may be impossible to utilize all CPUs\n";

        doing(args.verbosity, "Reading input", log);

        model m       = parse_bundle(rigid_name_opt, flex_name_opt, std::vector<std::string>(1, args.ligand_name));

        boost::optional<model> ref;
        done(args.verbosity, log);

        main_procedure(m, ref,
                args.out_name,
                args.score_only, args.local_only, args.randomize_only, false, // no_cache == false
                gd, args.exhaustiveness,
                weights,
                args.cpu, args.seed, args.verbosity, max_modes_sz, args.energy_range, log);
    }
    catch(file_error& e) {
